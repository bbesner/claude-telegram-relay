...
/**
 * v1.6.0: cheap preflight — scan project buckets for <sessionId>.jsonl.
 * Returns true if the session file exists and is a plausible Claude session,
 * false otherwise. Used before passing --resume so we can tell the user the
 * session is stale instead of letting Claude silently fail and then silently
 * start a fresh session on retry.
 */
function sessionFileExists(sessionId) {
  ...
  let buckets;
  try { buckets = fs.readdirSync(PROJECTS_DIR); } catch { return false; }
  for (const bucket of buckets) {
    const candidate = path.join(PROJECTS_DIR, bucket, sessionId + '.jsonl');
    try {
...
module.exports = { listAllSessions, formatSessionList, sessionFileExists };
