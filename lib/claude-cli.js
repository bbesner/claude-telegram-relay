...
// v1.9.0: permission mode passthrough. Print-mode Claude has no human to
// confirm prompts, so without an explicit mode the spawned subprocess is
// pinned to whatever's in the user's settings.json — typically tight.
// Operators who've gated the bot via ALLOWED_USER_IDS and want the relay
// to act with their full Claude Code surface can opt in to bypass.
const VALID_PERMISSION_MODES = ['default', 'acceptEdits', 'plan'];
function permissionArgs() {
  ...
  if (VALID_PERMISSION_MODES.includes(mode)) return ['--permission-mode', mode];
  log.warn('Ignoring invalid CLAUDE_PERMISSION_MODE', { value: mode, valid: ['bypass', ...VALID_PERMISSION_MODES] });
  return [];
}
...
function streamClaude(prompt, options = {}) {
  ...
  return new Promise((resolve) => {
    const args = ['-p', '--output-format', 'stream-json', '--verbose', ...permissionArgs()];
